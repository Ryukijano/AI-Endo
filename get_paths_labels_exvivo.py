...
def process_data(data_name, img_dir, label_dir, phase_dict):
    ...
    if os.path.isfile(label_file):
        ...
        phase_label = phase_label.infer_objects(copy=False)  # Retain old behavior
        phase_labels = phase_label["Phase"].tolist()
        
        # Verify lengths match
        if len(phase_label) > len(img_files):
            phase_label = phase_label.iloc[:len(img_files)]
...
def main():
    ...
    data_names = sorted([os.path.basename(x) for x in glob(pjoin(img_dir, "*"))])

    sorted_datas = {}
    for data_name in tqdm(data_names, desc="Processing ExVivo dataset"):
        data_name, data_dict = process_data(data_name, img_dir, label_dir, phase_dict)
        sorted_datas[data_name] = data_dict

    save_file = os.path.join(save_dir, "DATA_DICT_EXVIVO.pkl")
    ...
    for data_name, data_dict in sorted_datas.items():
        if data_dict['phase'] is None:
            print(f"{data_name}: No labels found")
        else:
            print(f"{data_name}: {len(data_dict['img'])} images, {len(data_dict['phase'])} labels")
        if len(data_dict['img']) != len(data_dict['phase']):
            print(f"  -> MISMATCH: {len(data_dict['img'])} images vs {len(data_dict['phase'])} labels")
